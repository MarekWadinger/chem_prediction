...
def plot_regression(inputs, predicted, expected):
    ...
    fig.add_trace(
        go.Scatter(
            x=expected, y=regr.predict(
                expected.reshape(-1, 1)).reshape(1, -1)[0],
            line=dict(color='red'),
...
# -----------------------------------------------------------------------------
PAGE_CONFIG = {
    "page_title": "ChemPropPrediction.io",
    "page_icon": ":atom_symbol:",
    "layout": "centered",
    "initial_sidebar_state": "collapsed",
    "menu_items": {
        "Get Help":
            "https://github.com/MarekWadinger/chem_prediction",
        "Report a bug":
            "https://github.com/MarekWadinger/chem_prediction/issues",
        "About":
            "# Prediction of chemical properties using regression analysis"
        }
    }
...
with col1:
    ...
    data_file = st.file_uploader(
        'Import data as .xyz file. Contains your sample molecules with '
        'properties.', '.xyz')
with col2:
    st.subheader('Pretrained model')
    model_file = st.file_uploader(
        'Import pretrained model as json file. Without pretrained model app '
        'trains a new one.', '.zip')

st.subheader('Parameters')
with st.form(key='Model Parameters'):
    # "soap", "mbtr" TODO: fix greedy tensorflow
    descriptors_list = ["cm", "acsf"]
...
with tf.device('/CPU:0'):
    if data_file:
        ...
        if submit_button:
            ...
            p1.success('Success!')
            st.plotly_chart(plot_regression(
                inputs=X_train,
            ...
            with open('my_model.h5.zip', 'rb') as f:
                button1.download_button(
                    'Download Model', f, 'my_model.h5.zip', "application/zip")
